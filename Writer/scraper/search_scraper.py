...
class SearchProductScraper:
    ...
    def product_details(self, product_name: str):
        details_prompt = f"""
        please introduce product {product_name}, give me every single detail about this product.
        """
        return self._call_llama_api(details_prompt)
    
    
    def product_pros_cons(self, product_name: str):
        pros_cons_prompt = f"""
                Extract product reviews and user feedback about {product_name}.
                Focus on actual user experiences, ratings, and opinions.
        """
        
        return self._call_llama_api(pros_cons_prompt)
    
    def product_sentiment(self, product_name: str):
        sentiment_prompt = f"""
        please analyze the sentiment of the product {product_name}.
        """
...
def main(product_name: str = "Meta Ray-Ban Smart Glasses"):
    ...
    product_sentiment = scraper.product_sentiment(product_name)
